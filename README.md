...
| `hunyuan`       | собирается в коде (`build_hunyuan_i2v_workflow`) | HunyuanVideo 1.5 I2V              |
...
### `hunyuan` (HunyuanVideo 1.5 I2V)
Собирается полностью в коде (`build_hunyuan_i2v_workflow` в `telegram_comfyui_bot.py`), отдельного JSON-файла нет. Добавлен как отдельный тестовый backend вместо прямой замены `ltx_sulphur`, потому что Hunyuan 1.5 использует другой граф: dual text encoder + CLIP vision + `HunyuanVideo15ImageToVideo`.
- `hunyuanvideo1.5_720p_i2v_fp16.safetensors` — `models/diffusion_models/`
- `hunyuanvideo15_vae_fp16.safetensors` — `models/vae/`
- `qwen_2.5_vl_7b_fp8_scaled.safetensors` и `byt5_small_glyphxl_fp16.safetensors` — `models/text_encoders/`
- `sigclip_vision_patch14_384.safetensors` — `models/clip_vision/`
- Настройки через env: `HUNYUAN_STEPS`, `HUNYUAN_CFG`, `HUNYUAN_SHIFT`, `HUNYUAN_FPS`, `HUNYUAN_WEIGHT_DTYPE`, `MAX_SECONDS_HUNYUAN`. Дефолты подобраны под интерактивную проверку на RTX 5070 12GB: 12 steps, 16 fps, лимит 4 секунды.
- **Качество:** UNet грузится как `fp8_e4m3fn` (fp16-файл 16GB не влезает в 12GB и стримится — fp8 освобождает ~8GB VRAM без потери качества кадра). Это позволяет поднять разрешение ближе к нативным 720p: пресеты `low/medium/high` = 448×768 / 512×896 / 576×1024 (проверено на 4s: пик ~11.75GB). Вернуть fp16-стриминг: `HUNYUAN_WEIGHT_DTYPE=default`.
- База HunyuanVideo 1.5 не обучена явной порнографии, и NSFW-лоры под архитектуру 1.5 пока нет (все Hunyuan NSFW-лоры на CivitAI — под оригинальную 13B-модель, несовместимы). Хардкор-промты в этом режиме получают подсказку уйти в 🌶 Eros; softcore/движение/лица Hunyuan делает отлично.
...
- Модель: `hf.co/mradermacher/Qwen2.5-32B-Instruct-abliterated-v2-i1-GGUF:Q4_K_M` — качается прямо с HF через Ollama:
  ...
  Принудительно работает на CPU (`options.num_gpu=0`, `keep_alive=0` в запросе) — без этого
  Ollama сама подгружала модель в VRAM (~9.7ГБ) и роняла ComfyUI с OOM прямо на текстовом
  энкодере. На CPU генерация сценария занимает ~90-100 сек, зато не конкурирует с видео-генерацией
  за единственную видеокарту (бот и ComfyUI — разные процессы, но GPU общий).
- Имя модели/таймаут настраиваются через `OLLAMA_SCENARIO_MODEL` / `OLLAMA_SCENARIO_TIMEOUT`.

### «🎙 Дубляж» (OpenVoice V2, замена голоса) + банк голосов
Кнопка в меню бота (`dub_voice_in_video` в `telegram_comfyui_bot.py`, вызывается из `send_result`
для `ltx_sulphur`/`ltx_eros`): после рендера видео → извлечь аудио → клонировать тембр голоса по
образцу (OpenVoice V2) → наложить обратно на видео. Один голос на весь ролик целиком (без
диаризации — для мультиперсонажного дубляжа с разными голосами это отдельная задача, не сделана).
Любое голосовое/аудио сообщение боту — предложит заменить голос по умолчанию (`DEFAULT_VOICE_NAME`,
`tati`) или добавить как новый (`voice2`, `voice3`, ...) в `voices/`; «🎙 Голос: <имя>» в меню —
выбор активного голоса из банка. Конвертируется ВСЯ смешанная дорожка целиком, БЕЗ разделения через
Demucs — Demucs обучен на музыке и путает стоны/не-речевые звуки, частично "роняя" их в фоновую
дорожку необработанными (был протестирован отдельно как вариант, результат хуже).
...
- `voices/*.mp3` (и `.wav`/`.ogg`/`.m4a`/`.oga`/`.flac`) — банк образцов голоса, личные данные
  пользователя, в git не входит (см. `.gitignore`). Положить хотя бы один файл с именем
  `<DEFAULT_VOICE_NAME>.<ext>` (по умолчанию `tati`), чтобы кнопка дубляжа заработала сразу после
  восстановления, остальные голоса добавляются через бота голосовыми сообщениями.
- `demucs` (pip, в venv бота) — установлен по ходу экспериментов, но в финальном пайплайне
  дубляжа не используется (см. выше почему); можно оставить или удалить, на работу бота не влияет.
