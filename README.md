...
### «🎙 Дубляж» (OpenVoice V2, замена голоса)
...
образцу (OpenVoice V2, голос задаётся `DUB_VOICE_SAMPLE_PATH`, по умолчанию `voices/tati.mp3`) →
наложить обратно на видео. Конвертируется ВСЯ смешанная дорожка целиком, БЕЗ разделения через
...
- `voices/tati.mp3` — образец голоса для клонирования, личные данные пользователя, в git не
  входит (см. `.gitignore`). Положить любой WAV/MP3 с голосом по пути из `DUB_VOICE_SAMPLE_PATH`
  (по умолчанию `./voices/tati.mp3`), чтобы кнопка дубляжа заработала.
